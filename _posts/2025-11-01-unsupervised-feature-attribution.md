...
But what if we don’t have a class? The answer is simple - we don't need it. We can just average all the activation maps at the last convolutional layer to get a generic saliency map. This “label-free” map doesn’t focus on any one class. It treats every learned feature as equally interesting, highlighting regions that strongly activate any of the high-level features in that layer. Essentially, it’s a visualization of “where the network is looking” in a class-agnostic sense. If the network has learned to detect meaningful patterns (objects, parts, textures), those patterns will light up their respective feature maps.
